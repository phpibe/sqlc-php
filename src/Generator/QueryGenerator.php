...
PHP;
    ...
    public function generate(array $queries): array
    {
        ...
        foreach ($groups as $group => $groupQueries) {
            ...
            // Generate the Query extension scaffold if ExtensionGenerator is available.
            // Mirror the DTO scope: when dto_scope is 'class' or 'method', group the
            // Query extension under the @class name (e.g. Extensions/Queries/CmsConfig/).
            $extData = null;
            if ($this->extGen !== null) {
                $groupSubdir = match ($this->dtoScope) {
                    'class', 'method' => $group,   // e.g. 'CmsConfig'
                    default           => null,       // flat — no subdirectory
                };
                $extData = $this->extGen->forQuery($className, $hostFqcn, $groupSubdir);
            }

            $code = $this->renderClass($className, $groupQueries, $extData);
            $files[$className] = [
                'className' => $className,
                'code'      => $code,
                'relPath'   => "{$className}.php",
            ];

            if ($extData !== null) {
                $files['ext:' . $className] = [
                    'className'    => $extData->traitName,
                    'code'         => null, // scaffold only — written once
                    'relPath'      => $extData->relPath,
                    'scaffoldCode' => $extData->scaffoldCode,
                    'isExtension'  => true,
                    'extType'      => 'queries',
                ];
            }
    ...
    /** @param QueryDefinition[] $queries */
    private function renderClass(string $className, array $queries, ?ExtensionData $extData = null): string
    {
        ...
        // Inject use TraitName; when extension is available
        $useTrait = $extData !== null
            ? "\n\n    use {$extData->traitName};"
            : '';
...
use PDO;
use Psr\Log\LoggerInterface;
use RuntimeException;{$paginatedImport}{$cursorImport}{$extensionImport}
use SqlcPhp\Query\QueryObject;
...
{{$useTrait}
     ...
     * Nests correctly when a transaction is already active — inner
     * withTransaction() calls are no-ops (the outer one owns the transaction).
     *
     * Usage in an extension trait:
     *
     *   // Example: createOrderWithItems(int \$userId, array \$items): int
     *   {
...
PHP;
    ...
    private function renderMethod(QueryDefinition $query): string
    {
        ...
        $main = match (true) {
            ...
            $query->returns->value === ':grouped'  => $this->renderGroupedMethod($query),
...
PHP;
    ...
    // -------------------------------------------------------------------------
    // :grouped — groups JOIN rows by a key column into typed sub-arrays
    // -------------------------------------------------------------------------
