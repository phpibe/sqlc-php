        ...
        foreach ($this->filters as $filter) {
            ...
            if ($filter->operator->isMultiValue()) {
                ...
                foreach ($values as $i => $v) {
                    $stmt->bindValue(":{$base}_{$i}", $v, $this->pdoType($v));
                }
            } elseif ($filter->operator->isTwoValue()) {
                $stmt->bindValue(":{$base}_from", $filter->value,   $this->pdoType($filter->value));
                $stmt->bindValue(":{$base}_to",   $filter->valueTo, $this->pdoType($filter->valueTo));
            } elseif ($filter->operator === FilterOperator::LIKE
                   || $filter->operator === FilterOperator::STARTS
                   || $filter->operator === FilterOperator::ENDS) {
                $stmt->bindValue(":{$base}", $filter->value, \PDO::PARAM_STR);
            } else {
                $stmt->bindValue(":{$base}", $filter->value, $this->pdoType($filter->value));
            }
