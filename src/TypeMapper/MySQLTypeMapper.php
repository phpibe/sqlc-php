...
class MySQLTypeMapper implements TypeMapperInterface
{
     ...
     * extracted before being passed to PDO::bindValue().
     *
     * PDO cannot auto-cast backed enums to their backing type — passing the
     * enum object directly throws "Object of class X could not be converted
     * to string/int". The binding must use `$param->value` instead.
     */
    public function needsValueExtraction(string $phpType): bool
    {
        $base = ltrim($phpType, '?\\');
        return $this->isBackedEnum($base);
    }
